  ...
  return (
    <div className="min-h-dvh bg-background text-text">
      ...
      <div className="flex flex-col lg:flex-row lg:h-[calc(100dvh-3rem)]">
        ...
        <div className={`
          flex-1 overflow-y-auto p-3 lg:p-6 pb-28 lg:pb-6
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 border-t border-border bg-surface/95 backdrop-blur px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] grid grid-cols-3 gap-3 z-10">
        {(['agents', 'tasks', 'content'] as const).map(panel => (
          <button
            key={panel}
            onClick={() => setActivePanel(panel)}
            className={`py-4 text-base font-semibold rounded-xl ${activePanel === panel ? 'btn-primary' : 'btn-secondary'}`}
          >
